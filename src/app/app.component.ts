...
export class AppComponent implements OnInit {
  ...
  players: string[] = ['Czerwony', 'Niebieski'];
  tiles1: number[][] = []; // red
  tiles2: number[][] = []; // blu
  states: any = ['', '💣', '⬛', '💥'];
  color: string[] = ['rgba(255, 0, 0, 0.05)', 'rgba(0, 0, 255, 0.05)'];
  selectedSize: number = 0;
  selectedDelay: number = 1;
  ...
  shotsToFirstHit: number = 0;
  firstHitOccurred: boolean = false;
  gameLogs: string =
    'Wygrany;Inteligentne statki;Procent trafien 1;Procent trafien 2;Nietrafione statki;Czas gry(s);Wielkosc planszy;Opoznienie;Konfiguracja statkow;Ilosc strzalow do pierwszego trafienia;\n';
  constructor() {}
  ...
  setShips() {
    const selectedConfig = this.shipConfigurations.find(
      (x) => x.id == this.configuration.value
    );
    if (selectedConfig) {
      this.shipsToPlace = selectedConfig.values;
    }
  ...
  shoot(playerIndex: number) {
    ...
    let shotKey: string;
    let availableShots =
      size * size - (playerIndex === 0 ? this.shots1.size : this.shots2.size);
    ...
    if (this.firstHitOccurred == null) {
      this.shotsToFirstHit++;
    }
    ...
    ships.forEach((shipLength) => {
      ...
      while (!placed) {
        ...
        const rowIndex = Math.floor(
          Math.random() *
            (isVertical
              ? this.selectedSize - shipLength + 1
              : this.selectedSize)
        );
        ...
        if (
          this.canPlaceShip(tiles, rowIndex, colIndex, shipLength, isVertical)
        ) {
          for (let i = 0; i < shipLength; i++) {
            tiles[isVertical ? rowIndex + i : rowIndex][
              isVertical ? colIndex : colIndex + i
            ] = 2; //set ship
          }
          placed = true;
        }
  ...
  canPlaceShip(
    ...
    for (let i = 0; i < shipLength; i++) {
      ...
      if (
        row >= this.selectedSize ||
        col >= this.selectedSize ||
        tiles[row][col] === 2
      ) {
    ...
    for (let i = 0; i < shipLength; i++) {
      ...
      for (const [dx, dy] of directions) {
        ...
        if (
          adjRow >= 0 &&
          adjRow < this.selectedSize &&
          adjCol >= 0 &&
          adjCol < this.selectedSize
        ) {
  ...
  isGameEnded(winningPlayer: number) {
    ...
    console.log(
      `Gra zakończona! Wygrał gracz ${this.players[winningPlayer - 1]}!`
    );
    console.log(`Czas gry: ${durationInSeconds.toFixed(2)} sekund.`);

    const hitPercentage1 =
      this.playerShots[0] > 0
        ? (this.playerHits[0] / this.playerShots[0]) * 100
        : 0;
    const hitPercentage2 =
      this.playerShots[1] > 0
        ? (this.playerHits[1] / this.playerShots[1]) * 100
        : 0;
    const remainingShips =
      winningPlayer === 1
        ? this.tiles1.flat().filter((tile) => tile === 2).length
        : this.tiles2.flat().filter((tile) => tile === 2).length;

    console.log(
      `Gracz 1 wykonał ${this.playerShots[0]} strzałów, z czego ${
        this.playerHits[0]
      } były celne (${hitPercentage1.toFixed(4)}%).`
    );
    console.log(
      `Gracz 2 wykonał ${this.playerShots[1]} strzałów, z czego ${
        this.playerHits[1]
      } były celne (${hitPercentage2.toFixed(4)}%).`
    );
    let inteligent = this.smartShips.value == true ? 'true' : 'false';
    let endGameString = `${this.players[winningPlayer - 1]};${inteligent};${hitPercentage1.toFixed(
      4
    )};${hitPercentage2.toFixed(
      4
    )};${remainingShips};${durationInSeconds.toFixed(2)};${
      this.selectedSize
    };${this.selectedDelay};[${this.shipsToPlace}];${this.shotsToFirstHit};\n`;
    this.gameLogs += endGameString;
    if(this.automatedLoops.value > 0){
      let value = this.automatedLoops.value - 1;
      this.automatedLoops.patchValue(value);
      setTimeout(()=>{
  ...
  downloadFile() {
    ...
    const filename = `${dateString}_${timeString}_SHIPS.txt`;
